"""
🎓🎯 CFB TD Props — full scorer board with filters.
"""
import os
import sys
...
st.set_page_config(page_title="CFB TD Props", page_icon="🎓", layout="wide")
st.title("🎓🎯 CFB TD Props — Scorer Board")
st.caption(
...
@st.cache_data(ttl=180, show_spinner="Scanning CFB TD props across 6 books...")
def _cached_scan(api_key):
    return scan(api_key, sport="americanfootball_ncaaf")


if st.button("🔄 Refresh", type="primary"):
    st.cache_data.clear()
    st.toast("Cache cleared — scanning CFB TD props...", icon="🔄")

with st.spinner("Scanning CFB TD props..."):
    try:
...
if not rows:
    st.warning(
        "No CFB TD props found right now — books may not have posted props yet "
        "(typically available Thursday/Friday before game week)."
    )
...
game_sel = st.sidebar.multiselect("Games", options=all_games, default=[],
                                  help="Leave empty to show all games")

player_search = st.sidebar.text_input("Player name", placeholder="e.g. Sanders, Dillon",
                                      help="Case-insensitive substring")

min_consensus = st.sidebar.slider("Min consensus %", 0, 80, 0, step=5)

max_price = st.sidebar.number_input("Max best price (American)", value=0, step=50,
                                    help="0 = no cap")
...
sort_by = st.sidebar.radio(
    "Sort by",
    ["Consensus % (most likely)", "Value Edge (best value)", "Sharp Gap (most steam)", "Best Price"],
    index=0,
)
...
with t1: _render(anytime)
with t2: _render(first_td)
with t3: _render(last_td)
