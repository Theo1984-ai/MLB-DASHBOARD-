"""
NFL Polymarket sharp-money scanner.

Same order-book depth logic as polymarket_sharp.py — just NFL tag on Gamma.
For each open NFL game-level market, measures bid-side imbalance within the
inside spread. Heavy skew = where limit-order book wants to bet at better
prices = sharp positioning signal.
"""
...
_UA = {"User-Agent": "nfl-dashboard/1.0"}
...
FUTURES_TOKENS = (
    "super bowl", "afc championship", "nfc championship", "conference championship",
    "division winner", "win the division", "win division", "playoff", "wildcard",
    "wild card", "season wins", "mvp", "offensive player", "defensive player",
    "rookie of the year", "coach of the year", "make the playoffs", "go to the",
    "reach the super bowl",
)


def _is_weekly_game(title, slug):
    """True if the market is a specific game (not a future or season-long market)."""
...
def _categorize(question, slug):
    ...
    if "first quarter" in q or "1st quarter" in q or "-1q-" in s:
        return "1st Quarter"
    if "touchdown" in q or " td " in q or "score a td" in q:
        return "Player TD"
...
def sharp_pick_label(parsed, skew_side):
    ...
    if mt == "player_td":
        ...
        body = f"{player} to score TD" if skew_side == "YES" else f"{player} NO TD"
        return body
    return skew_side


def parse_market_for_match(question, slug):
    """Decode an NFL Polymarket question into structured fields for SB matching."""
    q = question
...
def scan(min_volume=500, min_liquidity=20000, top_n=30, sleep_between=0.15,
    ...
    week_only=True: filters to markets whose gameStartTime is within
    the current NFL week (Mon–Mon window). Prevents off-season or
    future-week markets from showing up.
    """
    ...
    # NFL week window: allow games from last Monday through next Monday (all tz-aware ET)
    days_since_monday = today_et.weekday()  # Mon=0
    ...
    events = _get(f"{GAMMA}/events?closed=false&tag_slug=nfl&limit=200")
    weekly_markets = []
    filtered_future = 0
    filtered_started = 0
