"""
CFB Team Totals snapshot — on-demand, one file per week.

All CFB games this week go into one file keyed by the Monday that starts the week:
  cfb_team_totals_history/week_YYYY-MM-DD.json
...
BOOKS = "draftkings,fanduel,betmgm,bovada,williamhill_us,fanatics,betonlineag"
MARKET = "team_totals"
...
def _parse_game(event):
    ...
    for bm in event.get("bookmakers", []):
        key = bm.get("key")
        if key not in ("draftkings", "fanduel", "betmgm", "bovada", "williamhill_us", "fanatics", "betonlineag"):
            continue
